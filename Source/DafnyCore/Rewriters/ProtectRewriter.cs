...
public class ProtectRewriter(ErrorReporter r) : IRewriter(r) { // TODO: Figure out if the class can Extend Rewriter instead of IRewriter
  private class Replacer {
    public static Expression ReplaceExpr(Expression e) {
      Console.WriteLine($"Dafny IPM: {e.GetType().Name} not yet implemented.");
      return e;
    }
    public static Expression ReplaceExpr(StaticReceiverExpr e) => e;
    public static Expression ReplaceExpr(LiteralExpr e) => e;
    public static Expression ReplaceExpr(ThisExpr e) => ExpressionWrappedIn_Protect_Call(e);
    public static Expression ReplaceExpr(IdentifierExpr e) => ExpressionWrappedIn_Protect_Call(e);
    public static Expression ReplaceExpr(DatatypeValue e) => ExpressionWrappedIn_Protect_Call(e);
    public static Expression ReplaceExpr(NameSegment e) => ExpressionWrappedIn_Protect_Call(e);
    public static Expression ReplaceExpr(UnaryOpExpr e) => e.Op switch {
      UnaryOpExpr.Opcode.Cardinality  or
      UnaryOpExpr.Opcode.Allocated    or
      UnaryOpExpr.Opcode.Lit          or
      UnaryOpExpr.Opcode.Assigned     or
      UnaryOpExpr.Opcode.Not          => ReplaceExpr(e as Expression),
      UnaryOpExpr.Opcode.Fresh  or // apparently this is unreachable since unary expressions are handled by the "FreshExpr" subclass
      _                         => throw new UnreachableException(),
    };
    public static Expression ReplaceExpr(BinaryExpr e) => new BinaryExpr(e.Origin, e.Op, ReplaceExpr(e.E0 as dynamic), ReplaceExpr(e.E1 as dynamic));
    public static Expression ReplaceExpr(ChainingExpression e) {
      if (e.PrefixLimits.Any(l => l is not null)) {
        return ReplaceExpr(e as Expression);
      }
      return new ChainingExpression(e.Origin, e.Operands.ConvertAll(o => ReplaceExpr(o as dynamic) as Expression), e.Operators, e.OperatorLocs, e.PrefixLimits);
    }
    public static Expression ReplaceExpr(ParensExpression e) => new ParensExpression(e.Origin, ReplaceExpr(e.E as dynamic));
    public static Expression ReplaceExpr(DefaultValueExpression e) {
  ...
  private static Expression ExpressionWrappedIn_ProtectToProve_Call(Expression e) =>
    new ApplySuffix(e.Origin, null, new NameSegment(Token.NoToken, "_protectToProve", null), [
      new ActualBinding(null, Replacer.ReplaceExpr(e as dynamic), false),
  ...
  private static void ApplyChangesTo(TopLevelDecl d) {
    ...
    static void ReplaceExpressionInAssertStatement(AssertStmt a) {
      if (Attributes.Contains(a.Attributes, "itp")) {
        ...
        a.Expr = Replacer.ReplaceExpr(a.Expr as dynamic);
      }
    }
      ...
      case TopLevelDeclWithMembers decl:
        foreach (var member in decl.Members.OfType<MethodOrFunction>()) {
          foreach (var req in member.Req) {
            req.E = Replacer.ReplaceExpr(req.E as dynamic);
          }
          foreach (var ens in member.Ens) {
            if (Attributes.Contains(ens.Attributes, "itp")) {
              ...
              ens.E = Replacer.ReplaceExpr(ens.E as dynamic);
            }
          }
          switch (member) {
            case Function f:
